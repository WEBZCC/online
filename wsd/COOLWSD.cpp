...
void COOLWSD::innerInitialize(Application& self)
{
    ...
    SigUtil::signalLogSetFilePath(JailUtil::getJailedSignalLogFilePath(ChildRoot, "wsd"));
...
void extractSignalLogs()
{
    LOG_TRC("extractSignalLogs");

    // Open the signalLog file and copy its contents into the log.
    SigUtil::extractSignalLogFile(JailUtil::getJailedSignalLogFilePath(COOLWSD::ChildRoot, "wsd"));
    SigUtil::extractSignalLogFile(JailUtil::getJailedSignalLogFilePath(COOLWSD::ChildRoot, "frk"));

    std::unique_lock<std::mutex> newChildLock(NewChildrenMutex, std::defer_lock);
    if (newChildLock.try_lock())
    {
        for (const auto& child : NewChildren)
        {
            if (child)
            {
                LOG_DBG("Extracting signalLog from " << child->getPid() << " ("
                                                     << child->getJailId() << ')');
                const auto jailPath = JailUtil::getJailPath(COOLWSD::ChildRoot, child->getJailId());
                SigUtil::extractSignalLogFile(
                    JailUtil::getJailedSignalLogFilePath(jailPath, "kit"));
            }
        }
    ...
    if (docBrokersLock.try_lock())
    {
        for (const auto& pair : DocBrokers)
        {
            ...
            if (docBroker)
            {
                LOG_DBG("Extracting signalLog from " << docBroker->getPid() << " ("
                                                     << docBroker->getJailId() << ')');
                SigUtil::extractSignalLogFile(
                    JailUtil::getJailedSignalLogFilePath(docBroker->getJailRoot(), "kit"));
            }
        }
